...
export const generateArticle = async (req, res) => {
    try {
        ...
        if(plan !== 'premium' && free_usage >=10){
            return res.json({success:false, message:"Limit reached. Please upgrade to premium plan."})
        }
...
export const removeImageBckground = async (req, res) => {
    try {
        const{ userId } = req.auth();
...
export const removeImageObject = async (req, res) => {
  ...
  const prompt = `Remove object ${object} from image`;
